...
class ComponentScanner:
    ...
    PROPS_PATTERN = re.compile(
        r"(?:props\.|{\s*([^}]+)\s*}\s*=\s*props|:\s*([A-Z][a-zA-Z0-9]*Props))"
    )
    ...
    CALLBACK_PATTERN = re.compile(
        r"on[A-Z][a-zA-Z]*\s*[=:]|handle[A-Z][a-zA-Z]*\s*[=:]"
    )
    ...
    def scan(self) -> List[ComponentInfo]:
        ...
        for root, dirs, files in os.walk(self.source_path):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in {"node_modules", "__tests__", "test", "tests", ".git"}
            ]
    ...
    def _scan_file(self, file_path: Path):
        ...
        if "return" not in content or (
            "<" not in content and "jsx" not in content.lower()
        ):
    ...
    def _add_component(
        self, name: str, file_path: Path, content: str, component_type: str
    ):
        ...
        # Extract exports
        exports = re.findall(
            r"export\s+(?:default\s+)?(?:const|function|class)\s+(\w+)", content
        )
...
Examples:
  ...
  python test_suite_generator.py src/components/ -v
    ...
    parser.add_argument("source", help="Source directory containing React components")
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument("--template", help="Custom template file for test generation")
    parser.add_argument(
