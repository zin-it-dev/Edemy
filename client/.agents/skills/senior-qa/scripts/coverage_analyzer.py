...
class FileCoverage:
    ...
    def statement_pct(self) -> float:
        return (
            (self.statements[0] / self.statements[1] * 100)
    ...
    def branch_pct(self) -> float:
        return (
            (self.branches[0] / self.branches[1] * 100) if self.branches[1] > 0 else 100
        )

    @property
    def function_pct(self) -> float:
        return (
            (self.functions[0] / self.functions[1] * 100)
...
class CoverageParser:
    ...
    def _parse_istanbul_json(
        self, path: Path
    ) -> Tuple[Dict[str, FileCoverage], CoverageSummary]:
        ...
        for file_path, file_data in data.items():
            ...
            covered_branches = sum(
                sum(1 for h in hits if h > 0) for hits in b_hits.values()
            )
            ...
            if line.startswith("SF:"):
                ...
                if "node_modules" not in current_file:
                    files[current_file] = FileCoverage(
                        path=current_file,
                        statements=(
                            current_data["lines_hit"],
                            current_data["lines_total"],
                        ),
                        branches=(
                            current_data["branches_hit"],
                            current_data["branches_total"],
                        ),
                        functions=(
                            current_data["functions_hit"],
                            current_data["functions_total"],
                        ),
...
class CoverageAnalyzer:
    ...
    SERVICE_PATTERNS = [r"service", r"api", r"handler", r"controller", r"middleware"]

    def __init__(
        self, threshold: int = 80, critical_paths: bool = False, verbose: bool = False
    ):
    ...
    def analyze(
        self, files: Dict[str, FileCoverage], summary: CoverageSummary
        ...
        # Add summary stats
        stats = {
            "overall_statement_pct": (
                summary.statements[0] / summary.statements[1] * 100
            )
            if summary.statements[1] > 0
            else 100,
            "overall_branch_pct": (summary.branches[0] / summary.branches[1] * 100)
            if summary.branches[1] > 0
            else 100,
            "overall_function_pct": (summary.functions[0] / summary.functions[1] * 100)
            if summary.functions[1] > 0
            else 100,
            "overall_line_pct": (summary.lines[0] / summary.lines[1] * 100)
            if summary.lines[1] > 0
            else 100,
            "files_analyzed": summary.files_analyzed,
            "files_below_threshold": sum(
                1 for f in files.values() if f.line_pct < self.threshold
            ),
            "total_gaps": len(gaps),
            "critical_gaps": len(recommendations["critical"]),
            "threshold": self.threshold,
            "meets_threshold": (summary.lines[0] / summary.lines[1] * 100)
            >= self.threshold
            if summary.lines[1] > 0
            else True,
        }

        return gaps, {"recommendations": recommendations, "stats": stats}

    def _analyze_file(
        self, file_path: str, coverage: FileCoverage
    ) -> List[CoverageGap]:
        ...
        is_critical = any(
            re.search(pattern, file_path.lower()) for pattern in self.CRITICAL_PATTERNS
        )

        is_service = any(
            re.search(pattern, file_path.lower()) for pattern in self.SERVICE_PATTERNS
        )
        ...
        if coverage.line_pct < target_threshold:
            severity = (
                base_severity
                if coverage.line_pct < 50
                else self._lower_severity(base_severity)
            )
    ...
    def _lower_severity(self, severity: str) -> str:
        ...
        mapping = {"critical": "high", "high": "medium", "medium": "low", "low": "low"}
    ...
    def generate_text_report(
        ...
        if recs["critical"]:
            ...
            for rec in recs["critical"][:5]:
                ...
                if rec["lines"]:
                    lines.append(
                        f"    Uncovered lines: {', '.join(map(str, rec['lines'][:5]))}"
                    )
            lines.append("")
        ...
        below_threshold = [
            (path, cov) for path, cov in files.items() if cov.line_pct < threshold
        ]
        ...
        all_recs = recs["critical"][:2] + recs["high"][:2] + recs["medium"][:2]
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.output_format == "html":
            report = reporter.generate_html_report(
                files, summary, analysis, self.threshold
            )
        else:
...
Examples:
  ...
  python coverage_analyzer.py coverage/ --critical-paths
    ...
    parser.add_argument("coverage", help="Path to coverage file or directory")
    parser.add_argument(
        "--threshold",
        "-t",
        type=int,
    ...
    parser.add_argument(
        "--json", action="store_true", help="Output results as JSON (summary only)"
    )
