...
import CustomUserButton from './customuser-button';
    ...
    <>
      ...
      <header
        className={`sticky top-0 z-40 w-full text-white transition-all duration-300 ${
          isScrolled
            ? 'bg-black/10 dark:bg-white/5 backdrop-blur-md'
            : 'bg-transparent'
        }`}
      >
        {/* NAVBAR */}
            ...
            {menuData.map((item) =>
              item.submenu ? (
                <div key={item.title} className="relative group">
                  <button className="flex items-center gap-1.5 text-sm text-zinc-200 hover:text-slate-300 transition-colors border-0 py-2 font-medium">
                    {item.title}
                    <svg
                  ...
                  <div className="absolute top-full left-0 mt-2 w-48 bg-slate-900/60 dark:bg-zinc-950/40 border border-slate-200/10 backdrop-blur-2xl rounded-xl py-2 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    {item.submenu.map((sub) => (
                      <a
                        key={sub.title}
                        href={sub.path}
                        className="block px-4 py-2.5 text-sm text-slate-200 hover:text-white hover:bg-white/10 mx-1 rounded-lg transition-colors"
                      >
                        {sub.title}
                      </a>
                    ))}
